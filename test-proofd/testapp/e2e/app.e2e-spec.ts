...
describe('Starting tests for testapp', function() {
  ...
  it('website title should be testapp', () => {
    page.navigateTo('/');
  ...
  it('network-name should be test-proofd@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('test-proofd@0.0.1.bna');
    });
  });

  it('navbar-brand should be testapp',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('testapp');
    });
  });
